    ...
    return (
        <div
            ...
            <div className={`container fixed top-0 h-[85px] z-[98765678] flex justify-between items-center py-2 px-5 ${locale == 'en' ? "pr-6" : "pl-6"} gap-3`}>
                <Link href={`/${locale}`} className="logo flex items-center text-primary font-bold text-2xl">
                    <span className='inline-flex h-10 w-10 items-center justify-center rounded-md bg-primary text-white shadow-sm'>
                        <Utensils className='h-5 w-5' />
                    </span>
                    <span className='ml-2 text-zinc-950 rtl:mr-2 rtl:ml-0'>{translation.logo}</span>
                </Link>
                <Navbar translation={translation} />
                <SwitchLanguage />
                <Cart labels={translation.cart} />
            </div>
        </div>
    )
}
